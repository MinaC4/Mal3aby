  ...
  return (
    <div className="pitch-card group card cursor-pointer">
      ...
      <div className="relative h-52 overflow-hidden bg-gray-200 dark:bg-dark-700">
        ...
        {/* Rating badge */}
        <div className="absolute top-3 left-3 bg-white/90 dark:bg-dark-800/90 backdrop-blur-sm px-2.5 py-1 rounded-full flex items-center gap-1 shadow-sm">
      ...
      <div className="p-4">
        ...
        <p className="text-gray-500 dark:text-slate-400 text-xs mb-3 line-clamp-2 leading-relaxed">{pitch.description}</p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {pitch.amenities.slice(0, 3).map((amenity) => (
            <span
              key={amenity}
              className="bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-[10px] px-2 py-0.5 rounded-full font-medium border border-emerald-100 dark:border-emerald-800/50"
            >
              {amenity}
            </span>
          ))}
          {pitch.amenities.length > 3 && (
            <span className="bg-gray-100 dark:bg-dark-700 text-gray-500 dark:text-slate-500 text-[10px] px-2 py-0.5 rounded-full font-medium">
              +{pitch.amenities.length - 3}
            </span>
          )}
        </div>
